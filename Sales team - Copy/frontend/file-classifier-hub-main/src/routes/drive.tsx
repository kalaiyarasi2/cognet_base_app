      ...
      {activeMode === "local" ? (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
            ...
            <StatCard
              label="Drive Root"
              value={s?.drive_root ? "Mounted" : "—"}
              icon={HardDrive}
              hint={s?.drive_root || undefined}
              accent={s?.drive_root ? "success" : "muted"}
            />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <Panel title="Drive Folders" description="Override defaults from .env">
              <div className="space-y-3">
                <div>
                  <Label className="text-[11.5px]">Input Folder</Label>
                  <div className="flex gap-2 items-center mt-1">
                    <Input value={input} onChange={(e) => setInput(e.target.value)} className="h-8 font-mono text-[12px] flex-1" placeholder={s?.drive_input || "G:\\My Drive\\uploads"} />
                ...
                <div>
                  ...
                  <div className="flex gap-2 items-center mt-1">
                    <Input value={output} onChange={(e) => setOutput(e.target.value)} className="h-8 font-mono text-[12px] flex-1" placeholder={s?.drive_output || "G:\\My Drive\\sorted"} />
                ...
                <div className="flex justify-end">
                  <Button size="sm" onClick={runLocal} disabled={running || !s?.connected}>
                    <Play className="w-3.5 h-3.5" /> {running ? "Running…" : "Run on Drive"}
                  </Button>
                </div>
              </div>
            </Panel>

            <Panel title="Files in Drive" description={`${s?.pdf_count ?? 0} PDFs ready`}>
