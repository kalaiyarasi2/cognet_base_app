...
import { Download, FileJson, BarChart3, Building2, RotateCcw, HardHat, FileText, Table as TableIcon, Loader2, Brain, Copy, Merge, Check, X } from "lucide-react";
...
const ResultsPanel = ({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 text-muted-foreground gap-3">
        <div className="stage-pulse">
          <FileJson className="w-12 h-12 opacity-40 animate-pulse" />
        </div>
  ...
  const isLossRun = docType === "INSURANCE_CLAIMS" || docType === "INSURANCE";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {showMerge && (
          <Button
            size="sm"
            variant="secondary"
            className="h-8 text-[11px] font-bold bg-green-500/10 text-green-600 hover:bg-green-500/20 border-green-500/20 border"
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 max-w-[400px] mb-2">
