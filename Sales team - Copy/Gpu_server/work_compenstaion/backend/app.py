...
def extract_batch():
    ...
    for r in results:
        if r['success'] and 'data' in r and 'extracted_schema' in r['data']:
            schema_data = r['data']['extracted_schema']
            filename = r.get('filename', 'unknown')
            
            # Add source filename for traceability
            result_copy = schema_data.copy()
            result_copy['source_filename'] = filename
            all_results_data.append(result_copy)
    ...
    if all_results_data:
        try:
            ...
            print(f"✓ Merged results ({len(all_results_data)} applications) saved to {merged_filepath}")
    ...
    return jsonify({
        ...
        'total_applications': len(all_results_data) if all_results_data else 0
    })
